...
def acquire_dataset():
    ...
    # Define the path to the 'Datasets' folder in your project root
    data_path = os.path.join(os.getcwd(), 'Datasets')

    # Ensure the directory exists (to prevent errors if empty)
    if not os.path.exists(data_path):
        os.makedirs(data_path)

    # MedMNIST should handles the 'check-and-skip' logic internally:
    # If 'breastmnist.npz' exists in data_path, it loads it.
    # If not, it downloads it to that folder.
    
    
    # Download and split datasets
    train_dataset = BreastMNIST(split='train', download=True, root=data_path)
    val_dataset   = BreastMNIST(split='val', download=True, root=data_path)
    test_dataset  = BreastMNIST(split='test', download=True, root=data_path)
...
def svm_pipeline1(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    #flatten images -- SVM needs 1D feature vectors
